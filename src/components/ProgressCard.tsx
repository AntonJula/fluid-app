...
const GOAL_CELEBRATIONS = [
  {
    ...
    message: "You reached today’s hydration goal!",
  },
  {
    eyebrow: "Daily Goal Complete",
    message: "Small sips added up beautifully.",
  },
  {
    eyebrow: "Target Reached",
    message: "Your hydration rhythm is right on track.",
  },
  {
    eyebrow: "Hydration Win",
    message: "You showed up for yourself today.",
  },
  {
    eyebrow: "Goal Complete",
    message: "Another steady day in the flow.",
  },
  {
    eyebrow: "Flow Complete",
    message: "Today’s water goal is in the bag.",
  },
] as const;
...
function getGoalCelebration(key: string) {
  const hash = Array.from(key).reduce((value, character) => (value * 31 + character.charCodeAt(0)) >>> 0, 0);
  return GOAL_CELEBRATIONS[hash % GOAL_CELEBRATIONS.length];
}
...
export function ProgressCard({
  ...
  const goalCelebration = getGoalCelebration(celebrationKey);

  return (
    <Card
      glass="strong"
      data-fluid-media
      className="fluid-progress-hero flex w-full max-w-full min-w-0 flex-col items-center justify-center p-4 text-center shadow-xl min-[380px]:p-5 sm:p-7"
    >
      <div className="flex w-full items-start justify-between gap-3 min-[380px]:gap-4">
        <div className="min-w-0 text-left">
          <p className="font-ui text-cyan-50 text-base font-bold tracking-wide">Today&apos;s flow</p>
          <p className="font-ui mt-1 text-[0.66rem] font-semibold uppercase tracking-[0.24em] text-water-300/72">
            Daily hydration
          </p>
        </div>
        <div className="shrink-0 rounded-xl border border-[1.5px] border-cyan-100/18 bg-water-950/26 px-3 py-2 text-right shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] min-[380px]:rounded-2xl">
      ...
      <div className="relative mt-5 grid w-full grid-cols-[minmax(0,1fr)_auto] items-end gap-3 overflow-hidden rounded-[1.15rem] border border-cyan-100/14 bg-water-950/20 px-4 py-4 text-left shadow-inner min-[380px]:gap-4 min-[380px]:px-5">
        <span className="pointer-events-none absolute -right-8 -top-10 h-28 w-28 rounded-full bg-cyan-300/10 blur-3xl" />
        <div className="min-w-0">
          <div className="font-numeric text-5xl font-black leading-none text-white drop-shadow-xl sm:text-6xl">
            {(intake / 1000).toFixed(1)}
            <span className="font-ui text-2xl text-water-300/80 font-bold tracking-normal sm:text-3xl"> L</span>
          </div>
          <p className="font-body mt-2 text-sm font-semibold text-water-200/74">
            Rhythm set to {(safeGoal / 1000).toFixed(1)} L
          </p>
        </div>
        {goalAction && <div className="relative z-10 shrink-0">{goalAction}</div>}
      </div>

      <div className="mt-4 w-full rounded-[1.1rem] border border-[1.5px] border-water-300/14 bg-water-950/22 p-1.5 shadow-inner min-[380px]:rounded-3xl">
        <div className="h-3.5 overflow-hidden rounded-full bg-water-950/52">
          <div
            className={`h-full rounded-full transition-all duration-700 ease-out ${
              isGoalMet
                ? "bg-gradient-to-r from-cyan-300 via-emerald-200 to-cyan-200 shadow-[0_0_18px_rgba(153,246,228,0.45)]"
                : "bg-gradient-to-r from-water-500 via-cyan-300 to-emerald-200 shadow-[0_0_14px_rgba(34,211,238,0.2)]"
            }`}
      ...
      {!isGoalMet && (
        <div className="mt-4 grid w-full grid-flow-dense grid-cols-2 gap-2.5 text-left animate-in fade-in zoom-in duration-500 sm:gap-3">
          <div className="flex flex-col justify-center rounded-xl border border-[1.5px] border-water-300/12 bg-water-950/20 px-3 py-3 min-[380px]:rounded-2xl min-[380px]:px-4">
            <p className="font-ui text-water-300 text-[0.78rem] sm:text-sm font-bold tracking-wide">Left today</p>
          ...
          <div className="flex flex-col justify-center rounded-xl border border-[1.5px] border-water-300/12 bg-water-950/20 px-3 py-3 min-[380px]:rounded-2xl min-[380px]:px-4">
            <p className="font-ui text-water-300 text-[0.78rem] sm:text-sm font-bold tracking-wide">Quick target</p>
      ...
      {isGoalMet && (
        <div className="relative mt-5 flex w-full flex-col items-center justify-center overflow-hidden rounded-[1.1rem] border border-[1.5px] border-emerald-200/22 bg-gradient-to-br from-emerald-300/20 via-cyan-300/12 to-water-500/20 px-4 py-5 shadow-[0_0_34px_rgba(45,212,191,0.16)] animate-in fade-in zoom-in duration-500 min-[380px]:rounded-3xl min-[380px]:px-5 min-[380px]:py-6">
          ...
          <div className="relative z-10 flex items-center text-water-200">
            <p className="font-ui text-center text-[13px] font-bold uppercase tracking-[0.2em]">
              {goalCelebration.eyebrow}
            </p>
          </div>
          <p className="font-ui mt-2 max-w-[15rem] text-2xl font-black leading-tight text-white text-center relative z-10">
            {goalCelebration.message}
          </p>
          {overGoal > 0 && (
